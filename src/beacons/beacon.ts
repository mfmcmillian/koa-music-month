...
const beaconPositions = [
  Vector3.create(5.4, 1.72, 13.03),
  Vector3.create(2.53, 1.72, 14.58),
  Vector3.create(5.44, 1.72, 17.01)
]

export let allBeacons: Beacon[] = []

export const initBeacons = () => {
  allBeacons.forEach((beacon) => engine.removeEntity(beacon.entity))
  ...
  const homeBeacon = new Beacon(Vector3.create(29.01, 0.88, 14.88), beaconPositions.length, true)
...
export class Beacon {
  ...
  constructor(position: Vector3, beaconIndex: number, isHome = false) {
    ...
    GltfContainer.create(beacon, { src: isHome ? 'models/witch.glb' : 'models/witch.glb' })
  ...
  updateEnergyDisplays(previousEnergyLevel: number) {
    ...
    Material.setPbrMaterial(
      this.particles[this.energyLevel - 1],
